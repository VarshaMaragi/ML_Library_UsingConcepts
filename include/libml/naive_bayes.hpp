...
namespace libml {
        ...
        std::vector<int> classify(Data_type& x)
        {
                ...
                //Use the parameters obtained from the trained model
                std::map<int,int> c_prior;
                c_prior.insert(prior.begin(), prior.end());
                ...
                for(int i=0;i<logparameter.size();i++)
                {
                        ...
                        {
                                prob[i][j]=prob[i][j]+(c_prior[it->first]);
                        }
                        it++;
                }
