...
// w(k)(t+1) = w(k)(t) + e * gra
void train(logisticreg &p,data1 &myx,labels &myy
        
        ) {

    const int numberoffeatures=myx[0].size()+1;
        const int numberofexamples=myx.size();
    boost::numeric::ublas::matrix<double>& x(numberofexamples,numberoffeatures);
    boost::numeric::ublas::vector<double>& y(numberofexamples);

    for(int i=0;i<myx.size();i++)
    {
        for(int j=0;j<myy.size();j++)
        {
            x(i,j)=(double)myx[i][j];
        }
        y(i)=(double)myy[i];
    }
